...
if __name__ == '__main__':
    def run():
        dataclass = genetic_PSO.read_data.DataClass()
        attracties = [22,6,1,4,2,5,23,13,9]
        pso = PSO()

        tradeoffprice = 20 # euro/attractie
        startinghour = 84
        deadline = 117
        startinglocation = 6
        time1 = time.time()
        pso.crashPermutation(attracties, 1000, 10, 0.729, 2.025, 2.025, dataclass.averagewaitingtimes,
...
class PSO:
    ...
    def evaluate(self, particle, wachttijdentable, duurtijdtable, afstandtable, permutatie, deadline, startingtime,
        ...
        decisions = []
        for sc in particle:
            decisions.append(int(round(sc))) # float decision is rounded to nearest int
        leng = len(permutatie)
